  ...
  // #include "../include/Bluetooth.h" // DISABLED FOR TEST
  // #include "../include/BleProtocol.h" // DISABLED FOR TEST

  // --- SERIAL INPUT VARS ---
  String inputString = "";
  bool stringComplete = false;
  float serial_targetPan = 0.0f;
  float serial_targetTilt = 0.0f;
  ...
  const unsigned long TRACKING_TIMEOUT_MS = 2000; // INCREASED FOR MANUAL TEST (2s)
  ...
  // --- REPLACED: NEW SERIAL INPUT PROCESSING ---
  void processSerialInput() {
    ...
    if (stringComplete) {
        ...
        if (inputString.length() > 0) {
            ...
            if (cmd == 'M' || cmd == 'm') {
                Bt2_Mode_Flag = MANUAL_MODE;
                Serial.println("Switched to MANUAL_MODE via Serial");
            } else if (cmd == 'T' || cmd == 't') {
                Bt2_Mode_Flag = TRACKING_MODE;
                Serial.println("Switched to TRACKING_MODE via Serial");
            } else {
                // Check if it has a value
                if (inputString.length() > 2) {
                    float val = inputString.substring(2).toFloat();
                    
                    if (cmd == 'x' || cmd == 'X') {
                        targetPanAngle = val;
                        trackingDataReceived = true;
                        lastTrackingDataTime = millis();
                        Serial.printf("Set Pan Error via Serial: %.2f\n", targetPanAngle);
                    } else if (cmd == 'y' || cmd == 'Y') {
                        targetTiltAngle = val;
                        trackingDataReceived = true;
                        lastTrackingDataTime = millis();
                        Serial.printf("Set Tilt Error via Serial: %.2f\n", targetTiltAngle);
                    }
                }
            }
        }
  ...
  void setup() {
    ...
    // --- REMOVED BLUETOOTH INIT ---
    // Bluetooth::getInstance()->init("ESP32_Tracker", onBluetoothDataReceived);
    Serial.println("[TEST MODE] Bluetooth DISABLED. Use Serial Monitor.");
    Serial.println("Commands: x [val], y [val], T (Tracking), M (Manual)");
  ...
  //======================================================================================================================================================//
  void loop() {
    // --- REPLACED: Process Serial Input instead of BLE ---
    processSerialInput();
    ...
    // --- BTN1: Toggle Yaw/Roll ---
    if (btn1_state == HIGH && btn1_last == LOW) {
      // (Bỏ phần gửi Capture BLE)
    ...
    if (Bt2_Mode_Flag != last_mode) {
      ...
      if (Bt2_Mode_Flag == TRACKING_MODE) Tracking_Mode = CAPTURE_ZERO;
    ...
    switch (Bt2_Mode_Flag) {
      ...
      case(TRACKING_MODE): {
        // KHÔNG chuyển mode! Giữ velocity_openloop như MANUAL_MODE
        // Điều này đảm bảo chuyển mode mượt mà
        ...
        switch (Tracking_Mode) {
            case CAPTURE_ZERO: {
                ...
                Serial.printf("[TRACKING] Kp=1.2, Kd=0.08 | Starting from pos: %.2f, %.2f rad\n", 
            ...
            case BALANCE: {
                ...
                // Timeout check (Only if trackingDataReceived was ever set)
                // Serial input keeps trackingDataReceived true
                if (trackingDataReceived && (now_ms - lastTrackingDataTime > TRACKING_TIMEOUT_MS)) {
                    Serial.println("Tracking Timeout - Stopping");
                ...
                if (trackingDataReceived) {
                    ...
                    const float Kd = 0.08f;   // Derivative gain
                    const float MAX_VEL = 5.0f;   // deg/s max velocity
                    ...
                    float vel_pan = Kp * error_pan + Kd * filtered_d_pan;
                    float vel_tilt = Kp * error_tilt + Kd * filtered_d_tilt;
                    ...
                    // Debug log
                    static unsigned long last_send = 0;
                    // In ít hơn tí đỡ loạn
                    if (now_ms - last_send >= 200) { 
